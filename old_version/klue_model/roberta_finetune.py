...
from transformers import AutoModelForTokenClassification, AutoConfig, get_linear_schedule_with_warmup, AutoModel

import sys
sys.path.append("C:/Users/MATAGI/Desktop/Git/De-identification-with-NER")
sys.path.append("C:/Users/MATAGI/Desktop/Git/De-identification-with-NER/Utils")
sys.path.append("C:/Users/MATAGI/Desktop/Git/De-identification-with-NER/Utils/datasets_maker")
...
def main(cli_args):
    ...
    model = AutoModelForTokenClassification.from_pretrained("klue/bert-base", config=config)
    if args.load_state_dict:
        origin_model_st_dict = copy.deepcopy(model.state_dict())
        load_state_dict = torch.load(args.state_dict)
        update_cnt = 0
        for k, v in load_state_dict.items():
            if k in origin_model_st_dict.keys():
                origin_model_st_dict[k] = v
                update_cnt +=1

        model.load_state_dict(origin_model_st_dict)
        logger.info(f"Model State dict is loaded: {args.state_dict}, update_cnt: {update_cnt}")
    ...
    # load datasets
    train_dataset = NE_Datasets(path="./npy/train")
    dev_dataset = NE_Datasets(path="./npy/test")
    test_dataset = NE_Datasets(path="./npy/test")
