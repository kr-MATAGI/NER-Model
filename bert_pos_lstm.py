...
def main(cli_args):
    ...
    model = BERT_POS_LSTM.from_pretrained("klue/roberta-base", config=config)
    ...
    dev_seq_len = np.load("/".join(args.dev_npy.split("/")[:-1]) + "/dev_seq_len.npy")
    test_seq_len = np.load("/".join(args.train_npy.split("/")[:-1]) + "/test_seq_len.npy")
    print(f"train_seq_len.shape: {train_seq_len.shape}")
    print(f"dev_seq_len.shape: {dev_seq_len.shape}")
    print(f"test_seq_len.shape: {test_seq_len.shape}")

    train_pos_tag = np.load("/".join(args.train_npy.split("/")[:-1]) + "/train_pos_tag.npy")
    dev_pos_tag = np.load("/".join(args.dev_npy.split("/")[:-1]) + "/dev_pos_tag.npy")
    test_pos_tag = np.load("/".join(args.train_npy.split("/")[:-1]) + "/test_pos_tag.npy")
    print(f"train_pos_tag.shape: {train_pos_tag.shape}")
    print(f"dev_pos_tag.shape: {dev_pos_tag.shape}")
    print(f"test_pos_tag.shape: {test_pos_tag.shape}")
